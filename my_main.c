...
void my_main() {
  ...
  for (i=0;i<lastop;i++) {  
    ...
    case BOX:
      ...
      break;
    case SPHERE:
      add_sphere(tmp,op[i].op.sphere.d[0],op[i].op.sphere.d[1],op[i].op.sphere.d[2],op[i].op.sphere.r, 0.1);
      matrix_mult(peek(s),tmp);
      draw_polygons(tmp,t,g);
      tmp->lastcol = 0;
      break;
    case TORUS:
      add_torus(tmp, op[i].op.torus.d[0], op[i].op.torus.d[1], op[i].op.torus.d[2], op[i].op.torus.r0, op[i].op.torus.r1, 0.1);
      matrix_mult(peek(s), tmp);
      draw_polygons(tmp, t, g);
      tmp->lastcol = 0;
      break;
